...
import {getTreeView, getTreeViewEl, addEventListener,
...
export function enableHoverEvents() {
  ...
  disposables = new CompositeDisposable(
    ...
    addEventListener(getTreeViewEl(), 'mouseleave', () =>
      hideTreeView(config.hideDelay)
    ),

    addEventListener(getTreeViewEl().querySelector('.tree-view-resize-handle'), 'mousedown', event => {
...
export function disableHoverEventsUntilBlur() {
  if(!disposables) return;
  disableHoverEvents();
  addEventListener(getTreeView().list[0], 'blur', () => {
    clearFocusedElement();
    enableHoverEvents();
    hideTreeView();
  }, false, true);
