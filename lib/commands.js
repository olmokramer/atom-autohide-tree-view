...
import {showTreeView, hideTreeView, toggleTreeView,
  clearFocusedElement, resizeTreeView} from './autohide-tree-view.js';
...
export default function initCommands() {
  ...
  for(let action of ['expand', 'collapse']) {
    for(let isRecursive of ['', 'recursive-']) {
      disposables.add(
        atom.commands.add('atom-workspace', `tree-view:${isRecursive}${action}-directory`, resizeTreeView)
      );
    }
  }
  ...
  for(let direction of ['', '-right', '-left', '-up', '-down']) {
    disposables.add(
      atom.commands.add('atom-workspace', `tree-view:open-selected-entry${direction}`, didOpenFile)
    );
  }

  for(let i of [1, 2, 3, 4, 5, 6, 7, 8, 9]) {
    disposables.add(
      atom.commands.add('atom-workspace', `tree-view:open-selected-entry-in-pane-${i}`, didOpenFile)
    );
  }
