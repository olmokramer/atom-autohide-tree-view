...
export function migrateConfig() {
  ...
  for(let [ oldSetting, newSetting ] of settingsToMigrate) {
    let oldValue = atom.config.get(`autohide-tree-view.${oldSetting}`);
    if(oldValue == null) continue;
    atom.config.set(`autohide-tree-view.${newSetting}`, oldValue);
    atom.config.unset(`autohide-tree-view.${oldSetting}`);
  }
}
