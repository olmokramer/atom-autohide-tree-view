...
import {getTreeViewEl, isChildOf, addEventListener,
  addDelegatedEventListener} from './utils.js';
...
export function enableClickEvents() {
  ...
  disposables = new CompositeDisposable(
    ...
    addEventListener(getTreeViewEl(), 'click', event => {
    ...
    addDelegatedEventListener(getTreeViewEl(), 'click', ':not(.tree-view-scroller)', event => {
    ...
    addEventListener(document.body, 'click', event => {
      if(event.button != 0 || isChildOf(event.target, getTreeViewEl().parentNode)) return;
      clearFocusedElement();
      hideTreeView();
      uninvalidateNextClick();
    }),
