...
export function disableAutohide() {
  ...
  updateTreeView(config.pushEditor || !visible);
  return (visible ? animate(treeViewEl.clientWidth, 0, panel) : showTreeView()).then(() => {
...
function handleEvents() {
  disposables = new CompositeDisposable(
    ...
    domListener(document.body, 'focus', () => {
      if(visible) hideTreeView();
    }, {useCapture: true, delegationTarget: 'atom-text-editor'}),
...
export function showTreeView(delay = 0, shouldDisableHoverEvents = true) {
  ...
  storeFocusedElement();
  var animation = animate(getContentWidth(), delay);
  animation.then(hasFinished => {
    if(hasFinished) {
...
export function hideTreeView(delay = 0) {
  ...
  var animation = animate(config.minWidth, delay);
  animation.then(hasFinished => {
    if(hasFinished) {
      // focus the element that had focus when show() was called
      restoreFocus();
