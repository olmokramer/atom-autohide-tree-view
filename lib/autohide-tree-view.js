...
function handleEvents(touchEvents) {
  ...
  // package is enabled
  if(touchEvents) {
    disposables.add(onTouchSwipeLeft(swipeChange, swipeEnd));
    disposables.add(onTouchSwipeRight(swipeChange, swipeEnd));
  }
...
// true if target is a child of treeViewEl.parentNode
...
function swipeChange({source, deltaX}) {
  if(!getConfig('showOn').match('touch') || !isTreeView(source)) return;
  // show tree view contents
  treeView.scroller[0].style.display = 'block';
  // deltaX * -1 if the tree view is on the right side
  deltaX = deltaX * Math.pow(-1, atom.config.get('tree-view.showOnRightSide'));
...
// triggered after swiping the tree view
function swipeEnd({source}) {
  if(!getConfig('showOn').match('touch') || !isTreeView(source)) return;
  // call show/hide so the tree view will grow/shrink to full-width/hidden-width
  if(visible) { hide(0); } else { show(0); }
}
