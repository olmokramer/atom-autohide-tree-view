...
function swipeRightEnd({source, deltaX}) {
  if(!getConfig('showOn').match('touch') || !isTreeView(source)) return;
  if(atom.config.get('tree-view.showOnRightSide')) {
    hide(0);
  } else {
...
module.exports = {
  config: {
    showOn: {
      description: 'The type of event that triggers the tree view to show or hide. The touch events require atom-touch-events (https://atom.io/packages/atom-touch-events) to be installed.',
      type: 'string',
      default: 'hover',
      enum: [
        'hover',
