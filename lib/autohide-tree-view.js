...
// used variables
var SubAtom, disposables, treeView, treeViewEl, visible, focusedElement, hoverEventsEnabled, currentAnimation;
...
function handleEvents() {
  ...
  disposables.add(atom.config.onDidChange('autonhide-tree-view.hiddenWidth', update));
  ...
  disposables.add(atom.commands.add('atom-workspace', {
    // own commands
    'autohide-tree-view:toggle-push-editor': function() { toggleConfig('pushEditor'); },

    // core commands
    'tool-panel:unfocus': function() { hide(0); },
    ...
    'tree-view:reveal-active-file': function() { show(0, true); },
  ...
  for(var direction of ['', '-right', '-left', '-up', '-down']) {
    disposables.add(atom.commands.add('atom-workspace', `tree-view:open-entry${direction}`, function() { hide(0); }));
  }

  for(var i of [1, 2, 3, 4, 5, 6, 7, 8, 9]) {
    disposables.add(atom.commands.add('atom-workspace', `tree-view:open-selected-entry-in-pane-${i}`, function() { hide(0); }));
  }
...
function cacheFocusedElement() {
  focusedElement = document.activeElement;
}

// clear the reference to the focusedElement useful
// when we want to invalidate the next recoverFocus
...
module.exports = {
  config: {
    showOn: {
      ...
      type: 'string',
      default: 'hover',
      enum: ['hover', 'click', 'hover + click', 'none'],
      order: 0
    },
    showDelay: {
