...
import pinView, {
  activate as activatePinView,
  deactivate as deactivatePinView,
} from './pin-view.js';
...
function handleEvents() {
  ...
  disposables.add(getTreeViewEl(), 'click', '.entry.file', event => {
  ...
  disposables.add(getTreeViewEl().querySelector('.tree-view-resize-handle'), 'mousedown', event => {
    if(!isVisible() || event.button == 0) disableTreeViewShrinking();
  });
...
// shows the tree view
export function show(delay = 0, shouldDisableHoverEvents = true, shouldFocusTreeView = true) {
  visible = true;
  ...
  return animate(treeViewWidth, delay).then(hasFinished => {
    ...
    if(hasFinished && shouldFocusTreeView) focusTreeView();
    return hasFinished;
  });
}

// hides the tree view
export function hide(delay = 0, shouldRestoreFocus = true) {
  visible = false;
  ...
  return animate(getConfig('minWidth'), delay).then(hasFinished => {
    ...
    if(hasFinished && shouldRestoreFocus) restoreFocus();
    return hasFinished;
  });
}
...
// temporarily disables shrinking of the tree view
