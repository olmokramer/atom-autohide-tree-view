...
export function enable() {
  ...
  return Promise.resolve(
    ...
    if(!treeViewPkg) return atom.notifications.addError('autohide-tree-view: could not activate because the tree-view package doesn\'t seem to be loaded');
...
export function show(delay = 0, shouldDisableHoverEvents = true) {
  ...
  return animate(treeViewWidth, delay).then(hasFinished => {
    ...
    if(hasFinished && getConfig('autoFocusTreeView')) getTreeView().focus();
    return hasFinished;
  });
}
...
export function hide(delay = 0) {
  ...
  return animate(getConfig('minWidth'), delay).then(hasFinished => {
    ...
    if(hasFinished && getConfig('autoFocusTreeView')) restoreFocus();
    return hasFinished;
  });
}
