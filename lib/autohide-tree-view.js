...
function deinit() {
  ...
  update(true).then(function() {
    ...
    resetTreeView();
    [ disposables, initialized, visible,
...
function activate() {
  ...
  disposables2.add(window, 'resize', onDidResizeWindow);
  disposables2.add(atom.config.observe('autohide-tree-view.maxWindowWidth', onDidResizeWindow));
}
