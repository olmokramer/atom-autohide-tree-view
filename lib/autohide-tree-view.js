...
function enable() {
  ...
  Promise.resolve(
    ...
    createHoverElement();
    treeView.scroller[0].appendChild(pinView);
    handleEvents();
...
function createPinView() {
  pinView = document.createElement('div');
  pinView.classList.add('tree-view-pin-button', 'icon', 'icon-pin');
  updatePinView();
}

function destroyPinView() {
  treeView.removeChild(pinView);
  pinView = null;
}

function updatePinView() {
  pinView.style.display = getConfig('showPinButton') ? '' : 'none';
}

function pinTreeView() {
  pinView.classList.add('active');
  disable();
}

function unpinTreeView() {
...
function handleEvents() {
  enabledDisposable = new SubAtom(
    ...
    // update the display state of the pin view
    atom.config.onDidChange('autohide-tree-view.showPinButton', updatePinView),
  ...
  enabledDisposable.add(hoverElement, 'mouseenter', function onMouseEnter() {
...
function show(delay, shouldDisableHoverEvents = true) {
  ...
  return animate(getContentWidth(), delay).then(function(finished) {
...
function getContentWidth() {
  var contentWidth = treeView.list[0].clientWidth;
  var headerWidth = treeViewEl.querySelector('.header').lastChild.offsetLeft + treeViewEl.querySelector('.header').lastChild.offsetWidth;
  var pinViewWidth = pinView.clientWidth;
  return Math.max(contentWidth, headerWidth + pinViewWidth);
}
...
function swipeChange({ args: event, source, deltaX }) {
  ...
  newWidth = Math.min(getContentWidth(), Math.max(getConfig('hiddenWidth'), newWidth));
  // request the frame
...
export function activate() {
  ...
  disposable.add(pinView, 'click', function() {
    enabled ? pinTreeView() : unpinTreeView();
  });
