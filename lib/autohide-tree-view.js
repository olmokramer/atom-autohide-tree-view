...
function disable() {
  ...
  update(true).then(function() {
    ...
    resetTreeView();
    [ disposables, enabled, visible,
      focusedElement, hoverEventsEnabled,
      currentSwipe, currentAnimation ] = [];
  }).catch(logError);
}
...
function handleEvents() {
  ...
  disposables.add(atom.commands.add('atom-workspace', {
    ...
    'tree-view:reveal-active-file': () => process.nextTick(show),
    ...
    // tree-view-resizer package commands
...
function update(pushEditor) {
  ...
  return Promise.resolve().then(function updater() {
    ...
    treeViewEl.style.height = !pushEditor && statusBar ? `calc(100% - ${statusBar.clientHeight}px)` : '';
...
function onDidResizeWindow(maxWindowWidth) {
  ...
  if(!maxWindowWidth || window.innerWidth < maxWindowWidth) enable();
  else disable();
}
...
var staticDisposables;

function activate() {
  staticDisposables = new SubAtom();

  // these commands are handled here, because we don't want to dispose them on disable()
  staticDisposables.add(atom.commands.add('atom-workspace', {
  ...
  staticDisposables.add(window, 'resize', onDidResizeWindow);
  staticDisposables.add(atom.config.observe('autohide-tree-view.maxWindowWidth', onDidResizeWindow));
}
...
function consumeTouchSwipeLeftService(onDidTouchSwipeLeft) {
  staticDisposables.add(onDidTouchSwipeLeft(swipeChange, swipeEndLeft));
}

function consumeTouchSwipeRightService(onDidTouchSwipeRight) {
  staticDisposables.add(onDidTouchSwipeRight(swipeChange, swipeEndRight));
}
