...
function handleEvents() {
  ...
  disposables.add(atom.config.onDidChange('autohide-tree-view.minWidth', update));
...
function show(delay, shouldDisableHoverEvents = true) {
  ...
  return animate(getContentWidth(), delay).then(finished => {
    if(hoverEventsEnabled()) {
      getTreeViewEl().querySelector('.tree-view-autohide-hover-area').style.pointerEvents = 'none';
    }
...
function hide(delay) {
  ...
  enableHoverEvents();
  if(hoverEventsEnabled()) {
    getTreeViewEl().querySelector('.tree-view-autohide-hover-area').style.pointerEvents = '';
  }
