...
export function isChildOf(childNode, parentNode) {
  while(childNode.parentNode != document && childNode.parentNode != null) {
    if(isParent(childNode, parentNode))
      return true;
...
function isParent(childNode, parentNode) {
  switch(typeof parentNode) {
    case 'string':
      return childNode.parentNode.matches(parentNode);
    case 'object':
      return !!parentNode && childNode.parentNode == parentNode;
    default:
      return false;
  }
}
...
export function restoreFocus() {
  ...
  if(!focusedElement || activeElement == focusedElement || activeElement != getTreeView().list[0])
    return;
...
// focus the tree view with some logic
// around it so it won't focus if for
// example a modal panel is open
export function focusTreeView() {
  var {activeElement} = document;

  // check for modal panel
  if(isChildOf(activeElement, 'atom-panel.modal'))
    return;
