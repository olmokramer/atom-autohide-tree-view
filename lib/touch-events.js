...
export function consumeTouchEvents(touchEventsService) {
  ...
  if(getConfig('showOn').match('touch')) {
    atom.packages.activatePackage('atom-touch-events').then(() => enable());
  }
}
...
export function enable() {
  if(enabled || !atom.packages.isPackageActive('atom-touch-events')) return;
  enabled = true;

  disposables = new SubAtom();

  disposables.add(
    touchEvents.onDidTouchSwipeLeft(swipeChange, swipeEndLeft)
  );

  disposables.add(
    touchEvents.onDidTouchSwipeRight(swipeChange, swipeEndRight)
  );
}
