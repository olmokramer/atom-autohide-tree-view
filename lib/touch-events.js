...
export function consumeTouchSwipeLeftService(touchSwipeLeftService) {
  onDidTouchSwipeLeft = touchSwipeLeftService;
  if(getConfig('showOn').match('touch') && onDidTouchSwipeRight) enable();
}

export function consumeTouchSwipeRightService(touchSwipeRightService) {
  onDidTouchSwipeRight = touchSwipeRightService;
  if(getConfig('showOn').match('touch') && onDidTouchSwipeLeft) enable();
}
...
export function enable() {
  if(!onDidTouchSwipeLeft || !onDidTouchSwipeRight)
    return atom.notifications.addError('autohide-tree-view: atom-touch-events is not loaded, but it is required for touch events to work');

  if(enabled) return;
  enabled = true;

  disposables = new CompositeDisposable(
    onDidTouchSwipeLeft(swipeChange, swipeEndLeft),
    onDidTouchSwipeRight(swipeChange, swipeEndRight),
  );
}
